...
constexpr auto state_of(StateT state)
{
    if constexpr (!std::convertible_to<T, bool>) {
        return internal::GenericState<T>{std::move(state)};
    } else {
        return state;
    }
}
...
template <typename T>
constexpr auto nth_byte(std::integral T value, std::size_t index) noexcept -> std::uint8_t
{
    return (value & (0xff << (index * 8))) >> (index * 8);
}

template <typename T>
constexpr auto nth_byte(std::floating_point T value, std::size_t index) noexcept -> std::uint8_t
{
    ...
    return bytes[(std::endian::native == std::endian::low) ? (sizeof(T) - index) : index];
}

constexpr auto writer(std::same_as<std::uint8_t> auto ...bytes)
{
...
constexpr auto make(std::nullptr_t)
{
    constexpr auto null_byte = 0xc0;
    return writer(null_byte);
}

constexpr auto make(bool value)
{
...
constexpr auto make(std::int8_t value) noexcept
{
...
constexpr auto make(std::int16_t value) noexcept
{
...
constexpr auto make(std::int32_t value) noexcept
{
...
constexpr auto make(std::int64_t value) noexcept
{
...
constexpr auto make(std::uint8_t value) noexcept
{
    constexpr auto type_byte = 0xc4;
    return writer(
        type_byte,
...
constexpr auto make(std::uint16_t value) noexcept
{
    constexpr auto type_byte = 0xc5;
    return writer(
        type_byte,
...
constexpr auto make(std::uint32_t value) noexcept
{
    constexpr auto type_byte = 0xc6;
    return writer(
        type_byte,
...
constexpr auto make(std::uint64_t value) noexcept
{
    constexpr auto type_byte = 0xc7;
    return writer(
        type_byte,
...
constexpr auto make(float value)
{
...
constexpr auto make(double value)
{
...
constexpr auto make(std::string_view str)
{
    ...
    return compose_writers(
        writer(type_byte),
        std::move(size_writer),
        writer(str | std::ranges::views::transform([](auto byte) { static_cast<std::uint8_t>(byte); }))
    );
}
...
constexpr auto make_array(std::size_t size, F&& generator)
{
    ...
    return [=, gen=std::forward<F>(generator)](auto iterator) {
        ...
        for (const auto index : std::ranges::views::iota(size)) {
            gen(index, iterator);
        }
    };
}

constexpr auto make_array(std::ranges::sized_range const auto& range)
{
...
struct Date {
    ...
    constexpr auto serializer = make_serializer(
        {"year", &Date::year},
        {"month", &Date::month},
        {"day", [](const Date& obj) { return serialize_int(obj.day); }}
    );
};
...
struct Person {
    ...
    constexpr auto serializer = make_serializer(
        ...
        {"children", [](const Person& person) { serliaze_array(person.children); }}
    );
};
